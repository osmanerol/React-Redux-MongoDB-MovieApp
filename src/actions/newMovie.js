...
export function onNewMovieSubmit({title, cover}){
    return dispatch=>{
        dispatch({
            type:'NEW_MOVIE',
            payload:axios.post('http://localhost:5050/api/movies',{
                title,
                cover
            })
...
export function fetchMovie(id){
    return dispatch=>{
        dispatch({
            type:'FETCH_MOVIE',
            payload:axios.get(`http://localhost:5050/api/movies/${id}`)
...
export function onUpdateMovieSubmit({_id, title, cover}){
    return dispatch=>{
        dispatch({
            type:'NEW_MOVIE',
            payload:axios.put(`http://localhost:5050/api/movies/${_id}`,{
                title,
                cover
            })
            .then(response=>console.log(response))   
